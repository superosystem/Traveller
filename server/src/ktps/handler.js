...
async function copyFile(fromDir, fromFile, targetDir, targetFile) {
  try {
      // Copy file1
      await fs.copyFile(path.join(fromDir, fromFile), path.join(targetDir, targetFile));
...
const addImageKtp = async (request, h) => {
  try {
    ...
    const pyFilename = `ktp/${filenameCustom}`;

    writeCoordinates(payload.data, pyFilename);
    // writeCoordinates(payload.data, imageUrl);

    // Predictions
    const bodyPrediction = {
      filename: `${allName}ktp.json`,
    };

    await axios
    .post('http://localhost:5000/', bodyPrediction)
      .then((res) => {
        // console.debug(`Status: ${res.status}`);
        // console.debug('Body: ', res.data);
