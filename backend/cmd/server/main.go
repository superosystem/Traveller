...
import (
	...
	"github.com/superosystem/TrainingSystem/backend/config"
	"github.com/superosystem/TrainingSystem/backend/helper"
	"github.com/superosystem/TrainingSystem/backend/middleware"
	"github.com/superosystem/TrainingSystem/backend/routes"

	_dbMySQL "github.com/superosystem/TrainingSystem/backend/infrastructure/mysql"
	_dbRedis "github.com/superosystem/TrainingSystem/backend/infrastructure/redis"
)
...
func main() {
	...
	configRedis := _dbRedis.ConfigDB{
		...
		REDIS_USER:     config.GetConfig("REDIS_USER"),
	...
	// init jwt config
	jwtConfig := helper.NewJWTConfig(config.GetConfig("JWT_SECRET"))

	// init mailer config
	mailerConfig := helper.NewMailer(
	...
	storageConfig := helper.NewCloudStorage(storageClient, config.GetConfig("BUCKET_NAME"))
