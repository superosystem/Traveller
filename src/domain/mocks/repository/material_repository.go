// Code generated by mockery v2.20.0. DO NOT EDIT.

package mocks

import (
	mock "github.com/stretchr/testify/mock"
	domain "github.com/superosystem/trainingsystem-backend/src/domain"
)

// MaterialRepository is an autogenerated mock type for the MaterialRepository type
type MaterialRepository struct {
	mock.Mock
}

// CountByCourse provides a mock function with given fields: courseIds
func (_m *MaterialRepository) CountByCourse(courseIds []string) ([]int64, error) {
	ret := _m.Called(courseIds)

	var r0 []int64
	var r1 error
	if rf, ok := ret.Get(0).(func([]string) ([]int64, error)); ok {
		return rf(courseIds)
	}
	if rf, ok := ret.Get(0).(func([]string) []int64); ok {
		r0 = rf(courseIds)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]int64)
		}
	}

	if rf, ok := ret.Get(1).(func([]string) error); ok {
		r1 = rf(courseIds)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Create provides a mock function with given fields: materialDomain
func (_m *MaterialRepository) Create(materialDomain *domain.Material) error {
	ret := _m.Called(materialDomain)

	var r0 error
	if rf, ok := ret.Get(0).(func(*domain.Material) error); ok {
		r0 = rf(materialDomain)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// Delete provides a mock function with given fields: materialId
func (_m *MaterialRepository) Delete(materialId string) error {
	ret := _m.Called(materialId)

	var r0 error
	if rf, ok := ret.Get(0).(func(string) error); ok {
		r0 = rf(materialId)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// Deletes provides a mock function with given fields: moduleId
func (_m *MaterialRepository) Deletes(moduleId string) error {
	ret := _m.Called(moduleId)

	var r0 error
	if rf, ok := ret.Get(0).(func(string) error); ok {
		r0 = rf(moduleId)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

// FindById provides a mock function with given fields: materialId
func (_m *MaterialRepository) FindById(materialId string) (*domain.Material, error) {
	ret := _m.Called(materialId)

	var r0 *domain.Material
	var r1 error
	if rf, ok := ret.Get(0).(func(string) (*domain.Material, error)); ok {
		return rf(materialId)
	}
	if rf, ok := ret.Get(0).(func(string) *domain.Material); ok {
		r0 = rf(materialId)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(*domain.Material)
		}
	}

	if rf, ok := ret.Get(1).(func(string) error); ok {
		r1 = rf(materialId)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// FindByModule provides a mock function with given fields: moduleIds
func (_m *MaterialRepository) FindByModule(moduleIds []string) ([]domain.Material, error) {
	ret := _m.Called(moduleIds)

	var r0 []domain.Material
	var r1 error
	if rf, ok := ret.Get(0).(func([]string) ([]domain.Material, error)); ok {
		return rf(moduleIds)
	}
	if rf, ok := ret.Get(0).(func([]string) []domain.Material); ok {
		r0 = rf(moduleIds)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]domain.Material)
		}
	}

	if rf, ok := ret.Get(1).(func([]string) error); ok {
		r1 = rf(moduleIds)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

// Update provides a mock function with given fields: materialId, materialDomain
func (_m *MaterialRepository) Update(materialId string, materialDomain *domain.Material) error {
	ret := _m.Called(materialId, materialDomain)

	var r0 error
	if rf, ok := ret.Get(0).(func(string, *domain.Material) error); ok {
		r0 = rf(materialId, materialDomain)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

type mockConstructorTestingTNewMaterialRepository interface {
	mock.TestingT
	Cleanup(func())
}

// NewMaterialRepository creates a new instance of MaterialRepository. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewMaterialRepository(t mockConstructorTestingTNewMaterialRepository) *MaterialRepository {
	mock := &MaterialRepository{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
