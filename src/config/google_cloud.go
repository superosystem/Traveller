...
func (s *StorageConfig) UploadImage(ctx context.Context, objName string, file multipart.File) (string, error) {
	...
	imageDir := fmt.Sprintf("%s/%s", helper.IMAGES_DIR, objName)
	...
	imgUrl := fmt.Sprintf("%s/%s/%s", helper.STORAGE_URL, s.BucketName, imageDir)
...
func (s *StorageConfig) UploadVideo(ctx context.Context, objName string, file multipart.File) (string, error) {
	...
	videoDir := fmt.Sprintf("%s/%s", helper.VIDEOS_DIR, objName)
	...
	videoUrl := fmt.Sprintf("%s/%s/%s", helper.STORAGE_URL, s.BucketName, videoDir)
...
func (s *StorageConfig) UploadAsset(ctx context.Context, objName string, file multipart.File) (string, error) {
	...
	assignmentDir := fmt.Sprintf("%s/%s", helper.ASSIGNMENTS_DIR, objName)
	...
	assignmentUrl := fmt.Sprintf("%s/%s/%s", helper.STORAGE_URL, s.BucketName, assignmentDir)
...
func (s *StorageConfig) DeleteObject(ctx context.Context, objName string) error {
	...
	path := fmt.Sprintf("%s/%s/", helper.STORAGE_URL, s.BucketName)
